...
export default function Post({userId})  {
    ...
    return (
        <main className='min-h-dvh p-2 md:p-4 lg:p-6 bg-gray-100 m-4 p-4'>
            ...
            {posts.map(post => (
                <Link  key={post.id} href='#' className='space-y-2 group'>
                    <Image src='/j2.jpg' alt='blog' width={400} height={300} className='rounded-lg' />
                    <div className='flex justify-between text-sm text-gray-500'>
                        <span className='text-center text-blue-600 font-bold group-hover:text-blue-800'>{post.data.author}</span>
                        <span className='text-center text-blue-600 font-bold group-hover:text-blue-800'>{post.data.timecreated}</span>
                    </div>
                    <h1 className='font-bold text-lg'> {post.data.title}</h1>
                    <p className='text-sm'> {post.data.body}</p>
                </Link>
            ))}
            </div>
